...
import { FiLayout, FiShield, FiFileText, FiLogOut, FiMenu, FiUser } from "react-icons/fi";
...
export const AgentLayout = () => {
    ...
    const navItems = [
        ...
        { path: "/agent/claims", label: "Claims", icon: <FiFileText className="h-5 w-5" /> }
    ];
        ...
        <div className="flex h-screen w-screen overflow-hidden bg-slate-950 text-slate-100 font-sans">
            ...
            <aside className={`fixed inset-y-0 left-0 z-20 flex w-64 flex-col bg-slate-900 border-r border-slate-800 transition-transform duration-300 md:static md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="flex h-16 items-center px-6 border-b border-slate-800">
                    <span className="text-xl font-bold bg-gradient-to-r from-emerald-400 via-teal-400 to-indigo-400 bg-clip-text text-transparent">
                        Labmentix Agent
                    </span>
                </div>
